...
Copies from https://github.com/kafvtpnz/DeepNN/tree/master/lab1_perceptron
"""
import pandas as pd
...
eta = 0.001
errors = 0.0000000000000000001
i = 0
Wprev = np.random.randint(0, 2, size=(1 + hiddenSizes, outputSize)).astype(np.float64)
print(errors, not np.array_equal(Wprev, Wout))
while errors > 0 and not np.array_equal(Wprev, Wout):
    if (errors > 1):
        Wprev = np.random.randint(0, 2, size=(1 + hiddenSizes, outputSize)).astype(np.float64)
    errors = 0
    Wprev = np.copy(Wout)
